...
export class ExplicitDiscoveryCandidateProvider implements MinerCandidateProvider {
  ...
  constructor(private readonly source: DiscoverySource, private readonly bindings: DiscoveryBinding[]) {}
  async candidates(request: RouteRequest): Promise<MinerCandidate[]> {
    ...
    return this.bindings.filter((binding) => available.has(binding.sourceId)).map((binding) => structuredClone(binding.candidate));
  }
}
